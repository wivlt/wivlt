...
RSpec.describe Wivlt::Validator do
  describe '.input_valid?' do
    context 'when only description is present' do
      it 'should be valid' do
        command = '"this is a basic description"'
        expect(Wivlt::Validator.input_valid?(command)).to be_truthy
      end
    end
    ...
    context 'when description is empty' do
      it 'should be invalid' do
        ...
        expect { Wivlt::Validator.input_valid?(command) }.to raise_error(ArgumentError, 'invalid input')
      end
    end
